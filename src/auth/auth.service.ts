...
export class AuthService {
  ...
  async signUp(signupInstance: ISignupInstance) {
    ...
    try {
      ...
      user.occupation = signupInstance.occupation;
      user.industry = signupInstance.industry;
      user.country_of_residence = signupInstance.country_of_residence;
      user.otp = _otp;
  ...
  async resetPassword(req: ILoginInstance) {
    ...
    try {
      if (user.new_password_token == null) {
        user.password = await this.utilityService.generatePassword(
          password,
          salt,
        );
